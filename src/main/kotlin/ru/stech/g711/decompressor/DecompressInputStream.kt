...
class DecompressInputStream(inputStream: InputStream, useALaw: Boolean) : FilterInputStream(inputStream) {
    ...
    private val table: IntArray = if (useALaw) alawtable else ulawtable
    ...
    override fun read(b: ByteArray, off: Int, len: Int): Int {
        ...
        var length = len
        var value: Int
        val inb = ByteArray(length shr 1) // get A-Law or u-Law bytes
        length = `in`.read(inb)
        if (length == -1) {
            return -1
        }
        for (i in 0 until length) {
            value = table[inb[i].toInt() and 0x00FF]
            b[offset++] = (value shr 8 and 0x00FF).toByte() // little-endian
            b[offset++] = (value and 0x00FF).toByte()
        }
        return length shl 1
    }
